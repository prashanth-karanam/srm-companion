...
const ROCK_SOLID_PARALLEL_AUTH = `async function _scrapePortalDirectlyOnDevice(netid, password, captchaVal) {
    ...
    try {
        ...
        // 1. Submit Credentials to LoginServlet with explicit 8s mobile timeout
        ...
        console.log('[On-Device Scraper] Login verified! Scraping profile, attendance & timetable in parallel...');
        
        // 2. Fetch Profile, Attendance, and Timetable IN PARALLEL (Max Speed)
        const scrapeHeaders = {
            'Content-Type': 'application/x-www-form-urlencoded',
...
async function doAutoLogin(isBackgroundRefresh = false) {
    ...
    try {
        ...
        if (res && res.success) {
            ...
            // Re-enable button and display error on failure
            if (!isBackgroundRefresh) {
                ...
                fetchLiveCaptcha(true); // Always refresh captcha for clean retry
            }
            return false;
        }
    } catch (e) {
...
if (sIdx !== -1 && eIdx !== -1) {
    appJs = appJs.substring(0, sIdx) + ROCK_SOLID_PARALLEL_AUTH + '\n\n' + appJs.substring(eIdx);
    fs.writeFileSync(APP_JS_PATH, appJs, 'utf8');
    console.log('✅ Installed Rock-Solid Parallel Auth Engine with Clean Button Recovery!');
} else {
