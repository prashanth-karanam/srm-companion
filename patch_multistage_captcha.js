...
const MULTI_STAGE_LIVE_CAPTCHA = `// ─── 100% Real SRM Portal Captcha Engine (Multi-Stage Native Bridge) ─────────
...
async function fetchLiveCaptcha(force = false) {
    ...
    _currentCaptchaCode = ''; // 100% Real Live SRM Mode
    ...
    // Main Fetch Flow
    try {
        console.log('[Captcha] Fetching official loginManager/youLogin.jsp from SRM portal...');
        ...
        // Extract the real dynamic token from the HTML
        ...
        if (pageRes.data && typeof pageRes.data === 'string') {
            ...
            if (tokenMatch) {
                ...
                console.log('[Captcha] Acquired dynamic tokenized URL:', dynamicCaptchaUrl);
            }
        }
        ...
        console.warn('[Captcha] Primary portal fetch failed, trying proxy standby:', err);
    }
    ...
    // If fetch failed completely: show Tap to Retry button
    box.innerHTML = '<div onclick="fetchLiveCaptcha(true)" style="cursor:pointer;display:flex;align-items:center;justify-content:center;height:42px;width:140px;background:rgba(239,68,68,0.1);border-radius:6px;font-size:0.75rem;color:#ef4444;border:1px solid rgba(239,68,68,0.3);text-align:center;">⚠️ Tap to Retry</div>';
    _isFetchingCaptcha = false;
}

// Auto-warmup on device ready
document.addEventListener('deviceready', () => { setTimeout(() => fetchLiveCaptcha(true), 250); }, false);
window.addEventListener('load', () => { setTimeout(() => fetchLiveCaptcha(false), 400); }, false);
`;
...
if (sIdx !== -1 && eIdx !== -1) {
    appJs = appJs.substring(0, sIdx) + MULTI_STAGE_LIVE_CAPTCHA + '\n\n' + appJs.substring(eIdx);
    fs.writeFileSync(APP_JS_PATH, appJs, 'utf8');
    console.log('✅ Successfully installed Multi-Stage Live Captcha Engine!');
} else {
